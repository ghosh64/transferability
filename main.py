...
for i in range(n_devices):
    if args.transferability:
        training_data=dataset_cicids(device=i, train=True, separate_nodes=True)
    else: training_data=dataset_cicids(device=i,train=True)
...
for comm_round in range(n_rounds):
    ...
    for dev in range(n_devices):
        ...
        if not args.transferability:
            training_data=dataset_cicids(device=dev,train=True)
            val_data=dataset_cicids(device=dev,val=True)
        else:
            training_data=dataset_cicids(device=dev, train=True, separate_nodes=True)
            val_data=dataset_cicids(device=dev,val=True, separate_nodes=True)
        ...
        if args.transferability:
            test_data=dataset_cicids(device=dev, test=True, separate_nodes=True)
        else:test_data=dataset_cicids(device=dev, test=True)
        ...
        if (comm_round+1)%2==0:
            ...
            for testing_device in testing_set:
                dataset_test=dataset_cicids(device=testing_device,test=True) if not args.transferability else dataset_cicids(device=testing_device, test=True, separate_nodes=True)
